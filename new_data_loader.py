...
class GazeDataset(Dataset):
    ...
    def __getitem__(self, idx_input):
        ...
        self.hdf_processed = h5py.File(
            os.path.join("/data/data2/aruzzi/train", self.selected_keys[key]),
            "r",
            swmr=True,
        )
        assert self.hdf.swmr_mode

        idx_processed = self.find_image(idx)

        gaze_label = self.hdf_processed['face_gaze'][idx_processed, :]
        gaze_label = gaze_label.astype('float')
        head_label = self.hdf_processed['face_head_pose'][idx_processed, :]
